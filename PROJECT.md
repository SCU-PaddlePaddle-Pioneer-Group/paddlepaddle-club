...
AIGC:
  ...
  Label: '1'
  ProduceID: 'dcd36a20-a3e2-4b2d-8062-20ed71919c1b'
  PropagateID: 'dcd36a20-a3e2-4b2d-8062-20ed71919c1b'
  ReservedCode1: '2e08c07a-9d34-4fca-91c6-8ba8f6d96e5e'
  ReservedCode2: '2e08c07a-9d34-4fca-91c6-8ba8f6d96e5e'
---

# PaddlePaddle Club — 项目文档

> 版本：v1.6　·　状态：维护中　·　最近更新：2026-09
...
- 降低招募门槛：让新人无需复杂的注册/登录流程即可了解并联系社团

### 2.2 功能需求（现状）

| 模块 | 说明 | 实现位置 |
| --- | --- | --- |
| 首屏 Hero | 社团 slogan + 动画优化 | `layouts/index.html` |
| 关于我们 | 社团宣言 + 核心数据（成员/项目/分享） | `layouts/index.html` |
| 动态流（发推文） | 项目与活动合并为统一帖子流，按时间倒序展示，用 `type` 标签区分 | `layouts/index.html` |
| 联系我们 | 联系入口（mailto）；不设「加入社团」功能 | `layouts/index.html` |
| 404 页 | 兜底错误页，指向首页 | `layouts/404.html` |



#### 新增需求：首页跳转与落地页（JS 驱动）

当前首页多处链接为占位（指向 `#join` 锚点或 `mailto`），无法真正到达内容。本次将动态内容统一为帖子流并支持跳转；联系入口仅保留「联系我们」。**跳转链接统一由 JS 装配**。

| # | 需求 | 现状 | 目标 |
| --- | --- | --- | --- |
| 1 | 联系入口 | 仅保留「联系我们」，删除「加入社团 / 成为共创者」 | 「联系我们」直达邮箱（mailto），不设加入页面 |
| 2 | 动态帖子流跳转 | 项目卡片、活动条目均为占位链接 | 帖子流中每条帖子跳转到对应详情页（`/posts/<slug>/`） |

设计要点：

- **联系入口**：
  - 「联系我们」→ 邮箱直达（`mailto`），不设加入/报名页面
  - 「了解社团文化」→ 社团文化页（`/about/`）
...
- **首页合并**：原「项目矩阵」「活动日历」两个区块合并为一个「动态流」区块，按时间倒序渲染所有帖子。
...
- **区块划分**：首页由 3 个 `section` 组成（hero → about → feed（动态帖子流）），联系入口为「联系我们」的 mailto，不设「加入」区块。
- **主题系统**：CSS 变量定义于 `:root`，含色板（`--ink` 深蓝、`--acid` 荧光绿、`--cyan` 青）、字体、分隔线。换肤只需改变量。
- **内容渲染**：`index.html` 通过 Hugo 模板变量注入站点级配置（`.Site.Params.description`、`.Site.Title`）；动态内容（项目/活动帖子）由 content 存储 + JS 加载渲染，静态文案仍硬编码。

### 4.3 模板渲染机制

`index.html` 里用了少量 Hugo Go Template 语法，它们是在**构建期**被替换成最终值的：
...
全部样式在 `static/css/main.css`（单文件、压缩成几行）。核心技术点如下。
...
本站唯一的运行时 JS，负责**从 content 统一加载帖子数据**，渲染首页动态帖子流，并装配跳转（帖子详情）与「联系我们」入口。
...
| `content/about/` | 「了解社团文化」落地页；「联系我们」直接用 mailto，不设页面 |
...
1. **内容驱动化**：在动态帖子流基础上，继续把关于 / 联系等其余区块数据化到 `content/`，避免改 HTML。
2. **新增页面**：博客、团队成员页，复用现有设计系统。
3. **本地图片**：当前无素材依赖，引入时注意放到 `static/` 并压缩。
4. **CI 加 lint**：引入 HTML 校验（如 `html-validate`）到 CI，提升协作质量下限。
